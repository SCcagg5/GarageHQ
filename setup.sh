...
#
# Arguments:
#   --host    Required. The FQDN used in certificates (e.g., garage.local).
...
#
# Usage:
#   sh bootstrap-secrets-and-certs.sh --host example.com
#   sh bootstrap-secrets-and-certs.sh --host example.com --force
############################################
...
parse_args() {
  while [ $# -gt 0 ]; do
    case "$1" in
      --host)
        HOST="${2:-}"
        [ -n "$HOST" ] || fail "--host requires a value (e.g., --host garage.local)"
        shift 2
        ;;
      --force)
        FORCE=1
...
############################################
# Function: run_cert_generation
# Purpose: Invoke the certificate generation script with the detected shell.
# Notes:
#   - Passes --host and optionally --force.
############################################
run_cert_generation() {
  CERT_SCRIPT="traefik/certs/generate.sh"
  [ -f "$CERT_SCRIPT" ] || fail "$CERT_SCRIPT not found."

  echo "[certs] Running ($SELF_SHELL) $CERT_SCRIPT --host \"$HOST\"${FORCE:+ --force} ..."
  if [ "$FORCE" -eq 1 ]; then
    "$SELF_SHELL" "$CERT_SCRIPT" --host "$HOST" --force
  else
    "$SELF_SHELL" "$CERT_SCRIPT" --host "$HOST"
  fi
}
