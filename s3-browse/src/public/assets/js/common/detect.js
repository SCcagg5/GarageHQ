...
(function () {
  ...
  // --- Extension → language (copie fidèle de ta base) ---
  const EXT_TO_LANG = {
  ...
  // --- MIME → language (copie fidèle de ta base) ---
  const MIME_TO_LANG = [
  ...
  const XLS_EXT = ['xls','xlsx','xlsm','xlsb','xlt','ods','csv','tsv','numbers'];
  ...
  function isMarkdownExt(e){ return ['md','markdown','mdown','mkd','rmd'].includes(e); }
  ...
  // --- Type resolution for preview routing ---
  // Returns one of: 'image','video','audio','pdf','markdown','code','embed','download'
